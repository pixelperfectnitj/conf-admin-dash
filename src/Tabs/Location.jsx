...
const Location = () => {
    ...
    const handleSubmit = (e) => {
        ...
        axios.post(`${import.meta.env.VITE_API_URL}/location`, formData, {
            headers: {
    ...
    const handleDelete = (deleteID) => {
        axios.delete(`${import.meta.env.VITE_API_URL}/location/${deleteID}`, {
            headers: {
                Authorization: import.meta.env.VITE_API_KEY
            }
        })
    ...
    const handleEdit = (editIDNotState) => {
        axios.get(`${import.meta.env.VITE_API_URL}/location/${IdConf}`, {
            headers: {
                Authorization: import.meta.env.VITE_API_KEY
            }
        })
                                    ...
                                    {data && Object.keys(data).length !== 0 ? (
                                        <tr className="border-[1px] font-serif border-blue-500">
                                            ...
                                            <td className="p-1 text-center  flex justify-evenly">
                                                <button onClick={() => { handleEdit(data.id); setEditID(data.id); }} className="bg-yellow-500 hover:bg-yellow-700 text-white font-bold px-4 rounded focus:outline-none focus:shadow-outline"> Edit </button>{" "}
